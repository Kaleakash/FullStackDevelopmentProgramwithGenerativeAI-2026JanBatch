// Stack implementation without using predefined functions
class Stack {
    constructor(stackSize) {
        this.items = [];    // array to hold the stack elements
        this.stackSize = stackSize; // maximum size of the stack
        this.top = -1; // initialize top index to -1 indicating an empty stack
    }
    push(element) {
        if(this.items.length < this.stackSize) {
            this.top++; 
            this.items[this.top] = element; // add element to the top of the stack          
    ...
    pop() {
        if(this.top >= 0) {
            ...
            this.items.length--; // remove the top element
            this.top--; // decrement top index  
